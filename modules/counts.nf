...
process featureCounts_geneMult {
    ...
    script:
     def paired_flag = params.mode.toString().toLowerCase().replace('-', '_') == 'paired_end' ? '-p' : ''
     """
     featureCounts -T ${params.threads} ${params.featurecounts_extra_args} -M --fraction ${paired_flag} -s ${params.featurecounts_strand} -t ${params.featurecounts_gene_type} -g ${params.featurecounts_attribute} \
